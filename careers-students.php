...
<head>
	...
	<title>Metacube | Careers Students and Graduates </title>
	...
	<link href="assets/css/careers-students-style.css" rel="stylesheet">
	<link href="assets/css/careers-students-style-1280px.css" rel="stylesheet">
...
<body>
	...
	<div class="st-pusher no-fouc">
		...
		<div id="metacube-wrapper">
			...
			<!-------------------- CAREERS STUDENTS IMAGE BANNER -------------------->
			<div id="careers-students-image-banner" class="all_banners"></div>
			<!-------------------- CAREERS STUDENTS TEXT BANNER -------------------->
			<div id="main_wrapper">
				<div id="careers-students-text-banner">
					<div class="text">
						<h3>Students and Graduates</h3>
						<hr class="invisible" />
						<hr class="blue-underline" />					
						<p class="p3">We will be visiting your campus soon. Check out the dates below.</p>
					</div>
				</div>
				<div class="container">
					<!-------------------- CAREERS STUDENTS SECTION ONE -------------------->
					<div id="careers-students-section-one" class="mini-sections-color">
					...
					<!-------------------- CAREERS STUDENTS SECTION TWO -------------------->
					<div id="careers-students-section-two" class="mainContP">
						<div class="mainContDIV mainContDIVHidden">
							<img class="mainCont cssCount" src="assets/images/careers/visit-your-college.jpg" alt="">
						</div>
						<div class="inner-content">
							<h5>Want us to visit your college?</h5>
							<hr class="invisible">
							<hr class="white-underline">
							<p class="p3">We are constantly on the lookout for new talent. Placement coordinators can simply drop us a line, and we will contact you.</p>
							<button class="b3 button-ripple" onclick="sendmail();" style="cursor: pointer;"><span class="btn-text">GET IN TOUCH</span></button>
						</div>
					</div>
					<!-------------------- HORIZONTAL SPACER -------------------->
					<div class="horizontal-spacer"></div>
					<!-------------------- CAREERS STUDENTS SECTION THREE -------------------->
					<div id="careers-students-section-three" class="mainContP">
						<div class="mainContDIV mainContDIVHidden">
						...
						<div class="inner-content">
							...
							<button class="b3 button-ripple" onclick="setTimeout(function (){window.location.href='careers-professionals.php'},500);" style="cursor: pointer;"><span class="btn-text">SEE opportunities</span></button>
						</div>
					</div>
				</div>
				<!-------------------- FOOTER -------------------->
	...
	<script>
		function smallDeviceFormatting()
		{
			...
			$("#careers-students-section-one .row").css("height", columnHeight);
			$("#careers-students-section-one .row .content-panel").css("width", columnWidth);
		}
	...
	<script>
		function loaddata(type) {
            $.ajax({
                url: 'include/students.php?type=load&id=1',
                type: 'post',
                data: {  },
						...
						if (type == 'desktop') {
							...
							for(var a = 0; a <= 2; a++){
								if (obj.Records[t] != undefined) {
									content += '<div onclick="window.open(\'' + obj.Records[t].APPLY_LINK + '\');" class="content-panel">';
										content += '<table>';
											...
											content += '<tr class="text">';
												content += '<td>';
													content += '<p class="p1">' + obj.Records[t].SHORT_DESCRIPTION + '</p>';
												content += '</td>';
											content += '</tr>';
											content += '<tr class="subtext">';
												content += '<td>';
													content += '<p class="p4"><a href="javascript:;">' + obj.Records[t].CONTACT_PERSON + '</a></p>';
											content += '</tr>';
										content += '</table>';
									content += '</div>';
									if (a != 2) {
							...
							for(var a = 0; a <= 1; a++){
								if (obj.Records[t] != undefined) {
									content += '<div class="content-panel" onclick="window.open(\'' + obj.Records[t].APPLY_LINK + '\');">';
										content += '<table>';
											content += '<tr class="title">';
												content += '<td>';
													content += '<h6>' + obj.Records[t].TITLE + '</h6>';
		...
		function callemail(email) {
			...
			setTimeout(function(){ $('.closebtnIcon').click(); }, 100);
		}
